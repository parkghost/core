...
class PortainerCoordinator(
    ...
    async def update_data(self) -> dict[int, PortainerCoordinatorData]:
        ...
        for endpoint in endpoints:
            ...
            try:
                mapped_endpoints[endpoint.id] = await self._async_fetch_endpoint(
                    endpoint
                )
            except (PortainerConnectionError, PortainerTimeoutError) as err:
                _LOGGER.warning(
                    "Skipping endpoint %s (ID: %d) after error fetching data: %s",
                    endpoint.name,
                    endpoint.id,
                    repr(err),
                )
    ...
    async def _async_fetch_endpoint(
        ...
        """Fetch and assemble all data for a single endpoint."""
        (
            containers,
            docker_version,
            docker_info,
        ...
        if swarm_id:
            stack_requests.append(
                self.portainer.get_stacks(endpoint_id=endpoint.id, swarm_id=swarm_id)
            )
        ...
        local_images = dict(
            zip(
                ...
                await asyncio.gather(
                    *(
                        self._get_local_image(endpoint.id, str(container_inspect.image))
        ...
        for container in containers:
            ...
            prev_container = (
                prev_endpoint.containers.get(container_name) if prev_endpoint else None
            )
            ...
            image_status = (
                (
                    result.status
                    if (result := self.watcher.results.get((endpoint.id, container.id)))
                    else None
                )
                if self.watcher
                else None
        ...
        return PortainerCoordinatorData(
            id=endpoint.id,
            name=endpoint.name,
            endpoint=endpoint,
